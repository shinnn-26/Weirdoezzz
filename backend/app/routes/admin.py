...
def stats(_:bool=Depends(admin_required),db:Session=Depends(get_db)):
 ...
 return {'total':db.query(Registration).count(),'cs':db.query(Registration).filter(Registration.cluster=='CS').count(),'non_cs':db.query(Registration).filter(Registration.cluster=='Non-CS').count(),'today':db.query(Registration).filter(func.date(Registration.timestamp)==today).count()}
...
def list_regs(search:str='',cluster:str='',department:str='',year:str='',date:str='',_:bool=Depends(admin_required),db:Session=Depends(get_db)):
 ...
 if search:q=q.filter((Registration.full_name.ilike(f'%{search}%'))|(Registration.email.ilike(f'%{search}%'))|(Registration.phone.ilike(f'%{search}%')))
 ...
 fields=['id','registration_id','full_name','phone','department','year','student_type','dance_style','cluster','audition_date','status','sheet_synced']
 return [{c:getattr(r,c) for c in fields} for r in q.order_by(Registration.timestamp.desc()).all()]
